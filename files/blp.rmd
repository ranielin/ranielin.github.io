...
This walkthrough follows BLP (1999) in using a Taylor approximation to replace $-\alpha \ln(e^{m_t + \hat \sigma_y \nu_{iy}} - p_{jt})$ with $-\alpha \frac{p_{jt}}{y_{it}}$, where $y_{it} = e^{m_t + \hat \sigma_y \nu_{iy}}$ denotes the income of consumer $i$ in market $t$, which also normalizes the outside good utility to zero. 

Let $K$ denote the number of product characteristics and $L = 1$ denote the number of demographic variables. Adopting the notation of [Gandhi and Nevo (2021)](https://www.nber.org/system/files/working_papers/w29257/w29257.pdf), the indirect utility function can be re-written as $$u_{ijt} = \delta_{jt} + \mu_{ijt} + \epsilon_{ijt},$$ where 
