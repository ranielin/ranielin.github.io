...
This is a detailed walkthrough of a replication of the point estimates (the first "parameter estimates" column of Table IV) in Berry, Levinsohn, and Pakes (1995). A repository with all of the code can be found [here](https://github.com/ranielin/Berry-Levinsohn-and-Pakes-1995-Replication/). Note that this walkthrough is for expository purposes, i.e., to convey the basic fundamentals of the BLP demand estimation routine in an easy to understand format, and may not incorporate the most up-to-date practices for demand estimation.

### Model of Demand and Supply

The indirect utility function that consumer $i = 1, \dots I$ in receives from selecting product $j = 1, \dots, J_t$ in market $t = 1, \dots, T$, as specified in BLP (1995), is $$u_{ijt} = -\alpha \ln(e^{m_t + \hat \sigma_y \nu_{iy}} - p_{jt}) + x_{jt}^T \bar \beta + \xi_{jt} + \sum_k \sigma_k x_{jkt} \nu_{ik} + \epsilon_{ijt}$$ and $$u_{i0t} = -\alpha \ln(e^{m_t + \hat \sigma_y \nu_{iy}}) + \xi_{0t} + \sigma_0 \nu_{i0} + \epsilon_{i0t}$$ for the outside option, where $\alpha$ is the price coefficient, $m_t$ and $\hat \sigma_y$ are empirical income mean and standard deviation estimates, $\nu_{iy} \overset{\text{iid}}{\sim} N(0, 1)$ controls the distribution of income, $p_{jt}$ is the price of product $j$ in market $t$, $x_{jt}$ is a vector of characteristics of product $j$ in market $t$, $\bar \beta$ is a vector of linear parameters representing mean preferences for product characteristics, $\xi_{jt}$ is a latent demand shock, $\sigma_k$ are nonlinear parameters of consumers' taste heterogeneity, $\nu_{ik} \overset{\text{iid}}{\sim} N(0, 1)$ are random shocks to consumer $i$'s taste for product characteristic $k$, and $\epsilon_{ijt} \overset{\text{iid}}{\sim} T1EV$ is an idiosyncratic utility shock. 

This walkthrough follows BLP (1999) in using a Taylor approximation to replace $-\alpha \ln(e^{m_t + \hat \sigma_y \nu_{iy}} - p_{jt})$ with $-\alpha \frac{p_{jt}}{y_{it}}$, where $y_{it} = e^{m_t + \hat \sigma_y}$ denotes the income of consumer $i$ in market $t$, which also normalizes the outside good utility to zero, and adopts the notation of [Gandhi and Nevo (2021)](https://www.nber.org/system/files/working_papers/w29257/w29257.pdf). Letting $K$ denote the number of product characteristics and $L = 1$ denote the number of demographic variables, the indirect utility function can be re-written as $$u_{ijt} = \delta_{jt} + \mu_{ijt} + \epsilon_{ijt},$$ where 

* $\delta_{jt} = x_{jt} \beta_0 + \xi_{jt}$ is the mean utility of product $j$ in market $t$
* $\mu_{ijt} = (p_{jt}, x_{jt}) \cdot (- \Gamma D_{it} + \Sigma \nu_{it})$, where
    + $\Gamma = \begin{pmatrix} \alpha & 0 & \dotsm & 0 \end{pmatrix}^T$ is a $(K + 1) \times L$ matrix of non-linear parameters to be interacted with the $L \times 1$ demographic vector $D_{it} = \begin{pmatrix} D_{ilt} = \frac{1}{y_{it}} \end{pmatrix}$ 
    + $\Sigma = \begin{pmatrix} 0 \\ & \beta_{\nu}^1 \\ & & \ddots \\ & & & \beta_{\nu}^k \end{pmatrix}$ is a $(K + 1) \times (K + 1)$ matrix of non-linear parameters to be interacted with the $(K + 1) \times 1$ vector of random raste shocks $\begin{pmatrix} \nu_{it}^0 & \dotsm & \nu_{it}^k \end{pmatrix}^T$.
...
A few minor modifications are made to the data. The listed price in the dataset appears to have been demeaned, so the price is shifted upwards to reflect the prices in the original data. The trend term is also slightly modified to match the original data.
...
Prior to estimation, $R = 500$ random draws are taken separately in each market. The random draws $\hat F = \{\hat \nu_{it}, \hat D_{it}\}$ are simulated random taste shocks $\{\hat \nu_{it}\}_{i = 1}^R \sim N(0, 1)$ and incomes $\{\ln(\hat y_{it}) \}_{i = 1}^R \sim N(\mu_{dt}, \sigma_{dt}^2)$, where $\hat D_{it} = \frac{1}{\hat y_{it}}$ and $\mu_{dt}$ and $\sigma_{dt}$ are empirical estimates of the mean $m_t$ and standard deviation $\sigma_y$ of income. These estimates, which come from Census data, are taken from [Gentzkow and Shapiro (2016)](https://web.stanford.edu/~gentzkow/research/blp_replication.zip)'s replication files.
...
Model-implied market shares $$\sigma_{j}(\delta_t, x_t, p_t; \Gamma, \Sigma) = \int \frac{\exp(\delta_{jt} + \mu_{ijt})}{1 + \sum_{k = 1}^{J_t} \exp(\delta_{kt} + \mu_{ikt})} dF_D(D_{it}) dF_\nu(\nu_{it})$$ are numerically approximated using the random draws as $$\tilde \sigma_{jt}(\delta_t; \Gamma, \Sigma, p_t, \hat F) = \frac{1}{R} \sum_{i=1}^R \frac{\exp(\delta_{jt} + \mu_{ijt})}{\sum_{k = 1}^{J_t} \exp(\delta_{kt} + \mu_{ikt})}.$$ Note that different values of simulated draws $\{\hat \nu_{it}, \hat D_{it}\}$ will enter the approximated market share equation through the term $\mu_{ijt}$. The function ``compute_shares`` returns the approximated market-implied market shares given values of the mean utilities and non-linear parameters, market data, and random draws.

In BLP (1995), importance sampling is utilized in the approximation of market shares to reduce simulation error. This technique assigns different sampling weights to the unobservables. More details on this procedure can be found in BLP (1995) and BLP (1999), but for simplicity, this replication uses only a straightforward average over simulated draws to approximate market shares.
...
For a given value of the non-linear parameters $\Gamma$ and $\Sigma$, BLP (1995) provide a contraction mapping technique that recovers the unique value of mean utilities $\delta$ which will equate model-implied market shares with observed market shares, i.e., $\delta_t$ in each market $t$ such that $\tilde \sigma_t(\delta_t; \Gamma, \Sigma, p_t, \hat F) = \hat s_t$. The iterative technique begins with a starting guess $\delta_t^0$, which is taken to be $\ln(\frac{\hat s_t}{\hat s_{0t}})$, and updates $$\delta_t^{r+1} = \delta_t^r + \ln(\hat s_t) - \ln(\tilde \sigma_t(\delta_t^r; \Gamma, \Sigma, p_t, \hat F))$$ until $\max(\delta_t^r - \delta_t^{r + 1}) < tol$. Proof of convergence is provided in the appendix of BLP (1995).
...
1. Obtain the mean utilities $\delta_{jt}$ that equate model-implied market shares with observed shares using ``contraction_mapping`` and marginal costs using ``compute_mc``.
